...
///
/// By default, running Xenomorphic in development uses the development credentials
...
static XENOMORPHIC_DEVELOPMENT_USE_KEYCHAIN: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("XENOMORPHIC_DEVELOPMENT_USE_KEYCHAIN").is_ok_and(|value| !value.is_empty())
});

pub struct XenomorphicCredentialsProvider(pub Arc<dyn CredentialsProvider>);

impl Global for XenomorphicCredentialsProvider {}
...
pub fn init_global(cx: &mut App) {
    ...
    let provider = new(cx);
    cx.set_global(XenomorphicCredentialsProvider(provider));
}

pub fn global(cx: &App) -> Arc<dyn CredentialsProvider> {
    cx.try_global::<XenomorphicCredentialsProvider>()
...
fn new(cx: &App) -> Arc<dyn CredentialsProvider> {
    let use_development_provider = match ReleaseChannel::try_global(cx) {
        Some(ReleaseChannel::Dev) => {
            ...
            //
            // However, if the `XENOMORPHIC_DEVELOPMENT_USE_KEYCHAIN` environment
            // variable is set, we will use the actual keychain.
            !*XENOMORPHIC_DEVELOPMENT_USE_KEYCHAIN
        }
...
/// re-allow access to the system keychain when developing Xenomorphic.
