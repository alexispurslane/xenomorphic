...
use language::{
    ...
    ToOffset as _, XetaVersion, language_settings::all_language_settings, text_diff,
};
...
use xeta_prompt::{
    XetaFormat, format_xeta_prompt, get_prefill, parse_xeta2_model_output, stop_tokens_for_format,
    xeta1::{self, EDITABLE_REGION_END_MARKER},
};
...
pub fn request_prediction_with_zeta(
    ...
    let raw_config = store.xeta2_raw_config().cloned();
        ...
        async move {
            let local_zeta_version = custom_server_settings
                ...
                .and_then(|settings| match settings.prompt_format {
                    EditPredictionPromptFormat::Xeta(version) => Some(version),
                    EditPredictionPromptFormat::Infer => {
                        match settings.model.to_ascii_lowercase().as_str() {
                            "zeta" | "xeta1" => Some(XetaVersion::Xeta1),
                            "xeta2" => Some(XetaVersion::Xeta2),
                            "xeta2.1" => Some(XetaVersion::Xeta2_1),
                            _ => None,
                        }
                    }
                    _ => None,
                ...
                .unwrap_or_default();
            let xeta_format = raw_config
                .as_ref()
                .map(|config| config.format)
                .or(match local_zeta_version {
                    XetaVersion::Xeta1 => None,
                    XetaVersion::Xeta2 => Some(XetaFormat::V0211SeedCoder),
                    XetaVersion::Xeta2_1 => Some(XetaFormat::V0318SeedMultiRegions),
                })
                .unwrap_or_default();

            let cursor_offset = position.to_offset(&snapshot);
            let (full_context_offset_range, prompt_input) = xeta2_prompt_input(
                &snapshot,
                related_files,
                events,
                diagnostic_search_range,
            ...
            let formatted_prompt = format_xeta_prompt(&prompt_input, xeta_format);
                ...
                (if let Some(custom_settings) = &custom_server_settings {
                    ...
                    Some(match local_zeta_version {
                        XetaVersion::Xeta1 => {
                            ...
                            let prompt = xeta1::format_xeta1_from_input(
                                &prompt_input,
                            ...
                            let output_text = xeta1::clean_xeta1_model_output(&response_text);
                        ...
                        XetaVersion::Xeta2 | XetaVersion::Xeta2_1 => {
                            let Some(prompt) = formatted_prompt.clone() else {
                                return Ok((None, None));
                            };
                            let prefill = get_prefill(&prompt_input, xeta_format);
                            ...
                            let (response_text, request_id) = send_custom_server_request(
                                ...
                                prompt,
                                max_tokens,
                                stop_tokens_for_format(xeta_format)
                                    .iter()
                    ...
                    let Some(prompt) = format_xeta_prompt(&prompt_input, config.format) else {
                        return Ok((None, None));
                    };
...
pub(crate) fn active_buffer_diagnostics(
    ...
    snapshot
        ...
        .map(|entry| {
            ...
            xeta_prompt::ActiveBufferDiagnostic {
                severity,
...
pub fn xeta2_prompt_input(
    snapshot: &language::BufferSnapshot,
    related_files: Vec<xeta_prompt::RelatedFile>,
...
) -> (Range<usize>, xeta_prompt::XetaPromptInput) {
    ...
    let prompt_input = xeta_prompt::XetaPromptInput {
...
pub(crate) fn edit_prediction_accepted(
    ...
    let custom_accept_url = env::var("XENOMORPHIC_ACCEPT_PREDICTION_URL").ok();
    if store.xeta2_raw_config().is_some() && custom_accept_url.is_none() {
        return;
    }
