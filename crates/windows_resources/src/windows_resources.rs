...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Xenomorphic"),
        "preview" => ("app-icon-preview.ico", "Xenomorphic Preview"),
        "nightly" => ("app-icon-nightly.ico", "Xenomorphic Nightly"),
        _ => ("app-icon-dev.ico", "Xenomorphic Dev"),
    };
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "CompanyName", "Xenomorphic Contributors\0"
            VALUE "LegalCopyright", "Copyright 2022 - 2025 Zed Industries, Inc. (original); Xenomorphic contributors (fork)\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
