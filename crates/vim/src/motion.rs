...
impl Vim {
    pub(crate) fn search_motion(&mut self, m: Motion, window: &mut Window, cx: &mut Context<Self>) {
        if let Motion::XenomorphicSearchResult {
            prior_selections, ..
        } = &m
        {
            match self.mode {
...
impl Motion {
    fn default_kind(&self) -> MotionKind {
        ...
        match self {
            ...
            | SneakBackward { .. }
            | Jump { .. }
            | XenomorphicSearchResult { .. } => MotionKind::Exclusive,
    ...
    pub fn move_point(
        ...
        let (new_point, goal) = match self {
            ...
            XenomorphicSearchResult { new_selections, .. } => {
