    ...
    for (index, gpu) in metadata.gpus.iter().cloned().enumerate() {
        ...
        form = form
            .text(
                ...
                "A GPU found on the users system. May or may not be the GPU Xenomorphic is running on",
            )
    ...
    if let Some(active_gpu) = metadata.active_gpu.clone() {
        form = form
            .text(
                ...
                "The GPU Xenomorphic is running on",
            )
...
// users - except - we know internal users use Xenomorphic.
