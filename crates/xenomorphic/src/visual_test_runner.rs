...
//! Visual Test Runner
//!
//! This binary runs visual regression tests for Xenomorphic's UI. It captures screenshots
//! of real Xenomorphic windows and compares them against baseline images.
//!
...
#[cfg(target_os = "macos")]
fn main() {
    // Set XENOMORPHIC_STATELESS early to prevent file system access to real config directories
    // This must be done before any code accesses xenomorphic_env_vars::XENOMORPHIC_STATELESS
    // SAFETY: We're at the start of main(), before any threads are spawned
    unsafe {
        std::env::set_var("XENOMORPHIC_STATELESS", "1");
    }
...
use {
    ...
    xenomorphic_actions::OpenSettingsAt,
};
...
mod constants {
    ...
    pub const BASELINE_DIR: &str = "crates/xenomorphic/test_fixtures/visual_tests";

    /// Embedded test image (Xenomorphic app icon) for visual tests.
...
fn run_visual_tests(project_path: PathBuf, update_baseline: bool) -> Result<()> {
    ...
    // Test settings use Courier font, but we want the real Xenomorphic fonts for visual tests
    cx.update(|cx| {
        settings::init(cx);
    });
    ...
    // Initialize all Xenomorphic subsystems
    cx.update(|cx| {
        gpui_tokio::init(cx);
...
This is a test project for visual testing of Xenomorphic.
...
fn run_agent_thread_view_test(
    ...
    let send_future = thread.update(cx, |thread, cx| {
        thread.send(vec!["Show me the Xenomorphic logo".into()], cx)
    });
...
fn run_tool_permissions_visual_tests(
    ...
    use xenomorphic_actions::OpenSettingsAt;
