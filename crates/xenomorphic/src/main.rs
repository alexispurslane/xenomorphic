...
use crate::xenomorphic_app::{CrashHandler, OpenRequestKind, eager_load_active_theme_and_icon_theme};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Xenomorphic failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Xenomorphic failed to open a window: {e:?}. See https://zed.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Xenomorphic failed to launch")
                        .body(Some(
                            format!(
...
fn main() {
    ...
    #[cfg(target_os = "windows")]
    match util::get_xenomorphic_cli_path() {
        Ok(path) => askpass::set_askpass_program(path),
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("XENOMORPHIC_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    if stdout_is_a_pty() {
        xlog::init_output_stdout();
    } else {
        let result = xlog::init_output_file(paths::log_file(), Some(paths::old_log_file()));
    ...
    let version = option_env!("XENOMORPHIC_BUILD_ID");
    let app_commit_sha =
        option_env!("XENOMORPHIC_COMMIT_SHA").map(|commit_sha| AppCommitSha::new(commit_sha.to_string()));
    ...
    if args.system_specs {
        ...
        println!("Xenomorphic System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    let failed_single_instance_check = if *xenomorphic_env_vars::XENOMORPHIC_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::xenomorphic_app::listen_for_cli_connections(open_listener.clone()).is_err()
        }

        #[cfg(target_os = "windows")]
        {
            !crate::xenomorphic_app::windows_only_instance::handle_single_instance(open_listener.clone(), &args)
        }

        #[cfg(target_os = "macos")]
        {
            use xenomorphic_app::mac_only_instance::*;
    ...
    let should_install_crash_handler = matches!(
        env::var("XENOMORPHIC_GENERATE_MINIDUMPS").as_deref(),
        Ok("true" | "1")
    ) || *release_channel::RELEASE_CHANNEL
        != ReleaseChannel::Dev;
    ...
    app.run(move |cx| {
        ...
        settings::init(cx);
        xlog_settings::init(cx);
        xenomorphic_app::watch_settings_files(fs.clone(), cx);
        ...
        xenomorphic_app::init(cx);
        #[cfg(target_os = "macos")]
        xenomorphic_app::move_to_applications::init(cx);
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("xenomorphic://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(xenomorphic_actions::OpenSettings), cx),
                        Some(setting_path) => window.dispatch_action(
                            Box::new(xenomorphic_actions::OpenSettingsAt { path: setting_path }),
                            cx,
                        ),
                    })
                })
...
pub(crate) async fn restore_or_create_workspace(
    ...
    if let Some(multi_workspaces) = restorable_workspaces(cx, &app_state).await {
        ...
        // check, Xenomorphic would exit silently.
...
#[derive(Parser, Debug)]
#[command(name = "xeno", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/Xenomorphic`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/zed`.
    /// On Windows, the default is `%LOCALAPPDATA%\Xenomorphic`.
    ...
    /// prevents Xenomorphic from starting, so you can't run `zed: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,
    ...
    /// Used for recording minidumps on crashes by having Xenomorphic run a separate
    ...
    /// The PID of the Xenomorphic process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    ...
    /// Unix socket path for IPC with the parent Xenomorphic process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                || arg.starts_with("xenomorphic://")
                || arg.starts_with("xenomorphic-cli://")
                || arg.starts_with("ssh://")
