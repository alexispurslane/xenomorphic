...
pub use xenomorphic_actions::{Rerun, Spawn};
        ...
        fs.insert_tree(
            ...
            json!({
                ".xenomorphic": {
                    "tasks.json": r#"[
                        {
                            "label": "hello from $XENOMORPHIC_FILE:${XENOMORPHIC_ROW:$XENOMORPHIC_COLUMN",
                            "command": "echo",
                            "args": ["hello", "from", "$XENOMORPHIC_FILE", ":", "${XENOMORPHIC_ROW", ":", "$XENOMORPHIC_COLUMN"]
                        },
                        {
                            "label": "opened now: $XENOMORPHIC_WORKTREE_ROOT",
                            "command": "echo",
                            "args": ["opened", "now:", "$XENOMORPHIC_WORKTREE_ROOT"]
                        }
                    ]"#,
                },
            ...
            language_registry.add(Arc::new(
                ...
                .with_context_provider(Some(Arc::new(
                    ContextProviderWithTasks::new(TaskTemplates(vec![
                        ...
                        TaskTemplate {
                            label: "TypeScript task from file $XENOMORPHIC_FILE".to_string(),
                            command: "npm run build".to_string(),
                            ..TaskTemplate::default()
                        },
                        TaskTemplate {
                            label: "Another task from file $XENOMORPHIC_FILE".to_string(),
