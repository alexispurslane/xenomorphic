...
/// Inserts Xenomorphic-specific environment variables for terminal sessions.
/// Used by both local terminals and remote terminals (via SSH).
pub fn insert_zed_terminal_env(
    env: &mut HashMap<String, String>,
    version: &impl std::fmt::Display,
) {
    env.insert("XENOMORPHIC_TERM".to_string(), "true".to_string());
    env.insert("TERM_PROGRAM".to_string(), "xenomorphic".to_string());
...
#[derive(Clone)]
pub struct XenomorphicListener(pub UnboundedSender<AlacTermEvent>);

impl EventListener for XenomorphicListener {
    ...
    pub fn new(
        ...
        let fut = async move {
            ...
            let mut term = Term::new(
                config.clone(),
                &TerminalBounds::default(),
                XenomorphicListener(events_tx.clone()),
            );
...
pub struct Terminal {
    ...
    completion_tx: Option<Sender<Option<ExitStatus>>>,
    term: Arc<FairMutex<Term<XenomorphicListener>>>,
    term_config: Config,
...
impl Terminal {
    ...
    fn process_terminal_event(
        &mut self,
        event: &InternalEvent,
        term: &mut Term<XenomorphicListener>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        match event {
    ...
    fn make_content(term: &Term<XenomorphicListener>, last_content: &TerminalContent) -> TerminalContent {
    ...
    /// remote host, in case Xenomorphic is connected to a remote host.
    ...
    fn register_task_finished(&mut self, raw_status: Option<i32>, cx: &mut Context<Terminal>) {
        ...
        if !lines_to_show.is_empty() {
            ...
            // when Xenomorphic task finishes and no more output is made.
...
unsafe fn append_text_to_term(term: &mut Term<XenomorphicListener>, text_lines: &[&str]) {
    term.newline();
