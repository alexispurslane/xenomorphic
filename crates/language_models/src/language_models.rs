...
use language_model::{
    ConfiguredModel, LanguageModelProviderId, LanguageModelRegistry, XENOMORPHIC_CLOUD_PROVIDER_ID,
};
...
///
/// Prefers the Xenomorphic cloud provider so that, once the user is signed in, we
/// always pick a Xenomorphic-hosted model over models from other authenticated
/// providers in the environment. If the Xenomorphic cloud provider is authenticated
...
pub fn update_environment_fallback_model(cx: &mut App) {
    ...
    let fallback_model = {
        ...
        let cloud_provider = registry.provider(&XENOMORPHIC_CLOUD_PROVIDER_ID);
        if cloud_provider
            .as_ref()
