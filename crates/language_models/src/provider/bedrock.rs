...
// These environment variables all use a `XENOMORPHIC_` prefix because we don't want to overwrite the user's AWS credentials.
static XENOMORPHIC_BEDROCK_ACCESS_KEY_ID_VAR: LazyLock<EnvVar> = env_var!("XENOMORPHIC_ACCESS_KEY_ID");
static XENOMORPHIC_BEDROCK_SECRET_ACCESS_KEY_VAR: LazyLock<EnvVar> = env_var!("XENOMORPHIC_SECRET_ACCESS_KEY");
static XENOMORPHIC_BEDROCK_SESSION_TOKEN_VAR: LazyLock<EnvVar> = env_var!("XENOMORPHIC_SESSION_TOKEN");
static XENOMORPHIC_AWS_PROFILE_VAR: LazyLock<EnvVar> = env_var!("XENOMORPHIC_AWS_PROFILE");
static XENOMORPHIC_BEDROCK_REGION_VAR: LazyLock<EnvVar> = env_var!("XENOMORPHIC_AWS_REGION");
static XENOMORPHIC_AWS_ENDPOINT_VAR: LazyLock<EnvVar> = env_var!("XENOMORPHIC_AWS_ENDPOINT");
static XENOMORPHIC_BEDROCK_BEARER_TOKEN_VAR: LazyLock<EnvVar> = env_var!("XENOMORPHIC_BEDROCK_BEARER_TOKEN");
...
impl State {
    ...
    fn load_static_credentials(
        ...
        cx.spawn(async move |this, cx| {
            ...
            let (auth, from_env) = if let Some(bearer_token) = &XENOMORPHIC_BEDROCK_BEARER_TOKEN_VAR.value {
            ...
            } else if let Some(access_key_id) = &XENOMORPHIC_BEDROCK_ACCESS_KEY_ID_VAR.value {
                if let Some(secret_access_key) = &XENOMORPHIC_BEDROCK_SECRET_ACCESS_KEY_VAR.value {
                    if !access_key_id.is_empty() && !secret_access_key.is_empty() {
                        let session_token = XENOMORPHIC_BEDROCK_SESSION_TOKEN_VAR
                            .value
                            .as_deref()
                            .filter(|s| !s.is_empty())
                            .map(|s| s.to_string());
    ...
    fn get_region(&self) -> String {
        ...
        if let Some(region) = XENOMORPHIC_BEDROCK_REGION_VAR.value.as_deref() {
            if !region.is_empty() {
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let configured_label = match &auth {
            ...
            Some(BedrockAuth::IamCredentials { .. }) if env_var_set => {
                format!(
                    ...
                    XENOMORPHIC_BEDROCK_ACCESS_KEY_ID_VAR.name, XENOMORPHIC_BEDROCK_SECRET_ACCESS_KEY_VAR.name
                )
            }
            ...
            Some(BedrockAuth::ApiKey { .. }) if env_var_set => {
                format!(
                    ...
                    XENOMORPHIC_BEDROCK_BEARER_TOKEN_VAR.name
                )
            }
        ...
        let tooltip_label = if env_var_set {
            Some(format!(
                ...
                XENOMORPHIC_BEDROCK_ACCESS_KEY_ID_VAR.name,
                XENOMORPHIC_BEDROCK_SECRET_ACCESS_KEY_VAR.name,
                XENOMORPHIC_BEDROCK_SESSION_TOKEN_VAR.name,
                XENOMORPHIC_BEDROCK_BEARER_TOKEN_VAR.name
            ))
        ...
        v_flex()
            ...
            .child(Label::new("To use Xenomorphic's agent with Bedrock, you can set a custom authentication strategy through your settings file or use static credentials."))
...
impl ConfigurationView {
    fn render_static_credentials_ui(&self) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(
                Label::new(format!(
                    "You can also set the {}, {} and {} environment variables (or {} for Bedrock API Key authentication) and restart Xenomorphic.",
                    XENOMORPHIC_BEDROCK_ACCESS_KEY_ID_VAR.name,
                    XENOMORPHIC_BEDROCK_SECRET_ACCESS_KEY_VAR.name,
                    XENOMORPHIC_BEDROCK_REGION_VAR.name,
                    XENOMORPHIC_BEDROCK_BEARER_TOKEN_VAR.name
                ))
            ...
            .child(
                Label::new(format!(
                    ...
                    XENOMORPHIC_AWS_PROFILE_VAR.name,
                    XENOMORPHIC_AWS_ENDPOINT_VAR.name,
                    XENOMORPHIC_BEDROCK_SESSION_TOKEN_VAR.name
                ))
                .size(LabelSize::Small)
                .color(Color::Muted)
