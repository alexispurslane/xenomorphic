//! Baseline interface of Tasks in Xenomorphic: all tasks in Xenomorphic are intended to use those for implementing their own logic.
...
pub use debug_format::{
    ...
    Request, TcpArgumentsTemplate, XenomorphicDebugConfig,
};
...
pub use xenomorphic_actions::RevealTarget;
...
/// Contains all information needed by Xenomorphic to spawn a new terminal tab for the given task.
...
/// Variables, available for use in [`TaskContext`] when a Xenomorphic's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
impl FromStr for VariableName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let without_prefix = s.strip_prefix(XENOMORPHIC_VARIABLE_NAME_PREFIX).ok_or(())?;
        let value = match without_prefix {
            ...
            _ => {
                if let Some(custom_name) =
                    without_prefix.strip_prefix(XENOMORPHIC_CUSTOM_VARIABLE_NAME_PREFIX)
                {
...
pub const XENOMORPHIC_VARIABLE_NAME_PREFIX: &str = "XENOMORPHIC_";
const XENOMORPHIC_CUSTOM_VARIABLE_NAME_PREFIX: &str = "CUSTOM_";

impl std::fmt::Display for VariableName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::File => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}FILE"),
            Self::Filename => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}FILENAME"),
            Self::RelativeFile => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}RELATIVE_FILE"),
            Self::RelativeDir => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}RELATIVE_DIR"),
            Self::Dirname => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}DIRNAME"),
            Self::Stem => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}STEM"),
            Self::WorktreeRoot => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}WORKTREE_ROOT"),
            Self::Symbol => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}SYMBOL"),
            Self::Row => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}ROW"),
            Self::Column => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}COLUMN"),
            Self::SelectedText => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}SELECTED_TEXT"),
            Self::Language => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}LANGUAGE"),
            Self::RunnableSymbol => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}RUNNABLE_SYMBOL"),
            Self::PickProcessId => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}PICK_PID"),
            Self::MainGitWorktree => write!(f, "{XENOMORPHIC_VARIABLE_NAME_PREFIX}MAIN_GIT_WORKTREE"),
            Self::Custom(s) => write!(
                f,
                "{XENOMORPHIC_VARIABLE_NAME_PREFIX}{XENOMORPHIC_CUSTOM_VARIABLE_NAME_PREFIX}{s}"
            ),
        }
    }
}

/// Container for predefined environment variables that describe state of Xenomorphic at the time the task was spawned.
...
/// Keeps all Xenomorphic-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into Xenomorphic.
...
type XenomorphicEnvVariable = String;

struct EnvVariableReplacer {
    variables: HashMap<VsCodeEnvVariable, XenomorphicEnvVariable>,
    commands: HashMap<VsCodeCommand, XenomorphicEnvVariable>,
}

impl EnvVariableReplacer {
    fn new(variables: HashMap<VsCodeEnvVariable, XenomorphicEnvVariable>) -> Self {
        Self {
            variables,
    ...
    fn with_commands(
        mut self,
        commands: impl IntoIterator<Item = (VsCodeCommand, XenomorphicEnvVariable)>,
    ) -> Self {
    ...
    // Replaces occurrences of VsCode-specific environment variables with Xenomorphic equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->Xenomorphic hit, perform a substitution
