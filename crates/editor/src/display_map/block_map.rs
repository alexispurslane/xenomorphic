...
impl BlockMap {
    ...
    #[xtracing::instrument(skip_all, fields(edits = ?edits))]
    fn sync(
        &self,
        wrap_snapshot: &WrapSnapshot,
        mut edits: WrapPatch,
        ...
        while let Some(edit) = edits.next() {
            let span = xtracing::debug_span!("while edits", edit = ?edit);
            ...
            loop {
                let span = xtracing::debug_span!("decide where edit ends loop");
                let _enter = span.enter();
            ...
            for (block_placement, block) in blocks_in_edit.drain(..) {
                let span =
                    xtracing::debug_span!("for block in edits", block_height = block.height());
                let _enter = span.enter();
...
#[xtracing::instrument(skip(tree, wrap_snapshot))]
