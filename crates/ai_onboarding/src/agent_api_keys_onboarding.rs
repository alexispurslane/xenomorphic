...
use language_model::{IconOrSvg, LanguageModelRegistry, XENOMORPHIC_CLOUD_PROVIDER_ID};
...
impl ApiKeysWithProviders {
    ...
    fn compute_configured_providers(cx: &App) -> Vec<(IconOrSvg, SharedString)> {
        LanguageModelRegistry::read_global(cx)
            ...
            .iter()
            .filter(|provider| {
                provider.is_authenticated(cx) && provider.id() != XENOMORPHIC_CLOUD_PROVIDER_ID
            })
            ...
            .child(
                Button::new("configure-providers", "Configure Providers")
                    ...
                    .on_click(move |_, window, cx| {
                        window.dispatch_action(xenomorphic_actions::agent::OpenSettings.boxed_clone(), cx);
                    }),
            )
    }
}
