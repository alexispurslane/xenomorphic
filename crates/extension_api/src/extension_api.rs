//! The Xenomorphic Rust Extension API allows you write extensions for [Xenomorphic](https://zed.dev/) in Rust.
...
/// A result returned from a Xenomorphic extension.
...
pub trait Extension: Send + Sync {
    ...
    /// Locators are entities that convert a Xenomorphic task into a debug scenario.
    ///
    /// They can be provided even by extensions that don't provide a debug adapter.
    /// For all tasks applicable to a given buffer, Xenomorphic will query all locators to find one that can turn the task into a debug scenario.
    ...
    /// `dap_locator_create_scenario` if you're able to do that. Make sure to not fill out `build` field in that case, as that will prevent Xenomorphic from running second phase of resolution in such case.
...
/// Registers the provided type as a Xenomorphic extension.
///
...
macro_rules! register_extension {
    ($extension_type:ty) => {
        ...
        pub extern "C" fn __init_extension() {
            ...
            xenomorphic_extension_api::register_extension(|| {
                Box::new(<$extension_type as xenomorphic_extension_api::Extension>::new())
            });
        }
    };
}
...
#[doc(hidden)]
pub static XENOMORPHIC_API_VERSION: [u8; 6] = *include_bytes!(concat!(env!("OUT_DIR"), "/version_bytes"));
