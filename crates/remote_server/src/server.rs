        ...
        Commands::Version => {
            ...
            match release_channel {
                ReleaseChannel::Stable | ReleaseChannel::Preview => {
                    println!("{}", env!("XENOMORPHIC_PKG_VERSION"))
                }
                ReleaseChannel::Nightly | ReleaseChannel::Dev => {
                    let commit_sha =
                        option_env!("XENOMORPHIC_COMMIT_SHA").unwrap_or(release_channel.dev_name());
                    let build_id = option_env!("XENOMORPHIC_BUILD_ID");
...
pub static VERSION: LazyLock<String> = LazyLock::new(|| match *RELEASE_CHANNEL {
    ReleaseChannel::Stable | ReleaseChannel::Preview => env!("XENOMORPHIC_PKG_VERSION").to_owned(),
    ReleaseChannel::Nightly | ReleaseChannel::Dev => {
        let commit_sha = option_env!("XENOMORPHIC_COMMIT_SHA").unwrap_or("missing-zed-commit-sha");
        let build_identifier = option_env!("XENOMORPHIC_BUILD_ID");
        ...
        |_, _: TypedEnvelope<proto::GetCrashFiles>, _cx| async move {
            ...
            while let Some(child) = children.next().await {
                ...
                if extension == Some(OsStr::new("panic")) {
                    ...
                    if !filename.starts_with("xenomorphic") {
                        continue;
                    }
    ...
    let crash_handler = if should_install_crash_handler {
        Some(app.background_executor().spawn(crashes::init(
            crashes::InitCrashHandler {
                ...
                binary: "xenomorphic-remote-server".to_string(),
                release_channel: release_channel::RELEASE_CHANNEL_NAME.clone(),
                commit_sha: option_env!("XENOMORPHIC_COMMIT_SHA").unwrap_or("no_sha").to_owned(),
            },
            {
            ...
            |pid| paths::temp_dir().join(format!("xenomorphic-remote-server-crash-handler-{pid}")),
...
pub(crate) fn execute_proxy(
    ...
    if should_install_crash_handler {
        smol::spawn(crashes::init(
            ...
            |pid| paths::temp_dir().join(format!("xenomorphic-remote-server-proxy-crash-handler-{pid}")),
            // we are running outside gpui
            #[allow(clippy::disallowed_methods)]
            |duration| FutureExt::map(Timer::after(duration), |_| ()),
        ))
...
fn cleanup_old_binaries() -> Result<()> {
    ...
    let prefix = format!("xenomorphic-remote-server-{}-", release_channel);
...
fn is_new_version(version: &str) -> bool {
    semver::Version::from_str(version)
        .ok()
        .zip(semver::Version::from_str(env!("XENOMORPHIC_PKG_VERSION")).ok())
