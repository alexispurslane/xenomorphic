...
use xenomorphic_actions::{ExtensionCategoryFilter, Extensions};
...
pub fn init(cx: &mut App) {
    cx.on_action(|action: &xenomorphic_actions::theme_selector::Toggle, cx| {
        let action = action.clone();
        with_active_or_new_workspace(cx, move |workspace, window, cx| {
            toggle_theme_selector(workspace, &action, window, cx);
        });
    });
    cx.on_action(|action: &xenomorphic_actions::icon_theme_selector::Toggle, cx| {
        let action = action.clone();
        with_active_or_new_workspace(cx, move |workspace, window, cx| {
...
fn toggle_theme_selector(
    ...
    toggle: &xenomorphic_actions::theme_selector::Toggle,
    window: &mut Window,
...
fn toggle_icon_theme_selector(
    workspace: &mut Workspace,
    toggle: &xenomorphic_actions::icon_theme_selector::Toggle,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    let fs = workspace.app_state().fs.clone();
...
mod tests {
    ...
    fn open_theme_selector(
        ...
        cx.dispatch_action(xenomorphic_actions::theme_selector::Toggle {
            themes_filter: None,
        });
        cx.run_until_parked();
